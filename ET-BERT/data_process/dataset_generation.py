...
word_dir = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "corpora")) + "/"
...
def convert_pcapng_2_pcap(pcapng_path, pcapng_file, output_path):
    ...
    # Use the Linux 'editcap' (ensure it is installed via: sudo apt install wireshark-common)
    cmd = "editcap -F pcap %s %s"
    command = cmd % (os.path.join(pcapng_path, pcapng_file), pcap_file)
    os.system(command)
    return 0

def split_cap(pcap_path, pcap_file, pcap_name, pcap_label='', dataset_level = 'flow'):
    # Replace all "/" with "/" or os.path.join
    base_split_path = os.path.join(pcap_path, "splitcap")
    if not os.path.exists(base_split_path):
        os.mkdir(base_split_path)
        
    output_dir = os.path.join(base_split_path, pcap_label)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    
    # Resolve absolute path to remove /../ which can confuse mono
    splitcap_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "SplitCap.exe"))
    
    if not os.path.exists(splitcap_path):
        print(f"\n[CRITICAL ERROR] SplitCap.exe is missing! Please place it exactly here:\n{splitcap_path}\n")
        sys.exit(1)
        
    cmd = f"mono '{splitcap_path}' -r '{pcap_file}' -o '{output_dir}' -s session -p 1000"
    os.system(cmd)
    return output_dir
...
def get_burst_feature(label_pcap, payload_len, write_to_file=True):
    feature_data = []
    ...
    try:
        ...
        pass
    return "" if not write_to_file else 0
...
def generation(pcap_path, samples, features, splitcap = False, payload_length = 128, payload_packet = 5, dataset_save_path = "./ex_results/", dataset_level = "flow"):
    print(f"Checking for existing dataset at: {dataset_save_path}dataset.json")
    ...
    for parent, dirs, files in os.walk(pcap_path):
        if label_name_list == []:
            label_name_list.extend([d for d in dirs if d not in ['splitcap', 'dataset', 'packet', 'result']])
                ...
                if splitcap:
                    for file in ff:
                        if not (file.endswith('.pcap') or file.endswith('.pcapng')):
                            continue
                        session_path = (split_cap(pcap_path, p + "/" + file, os.path.splitext(file)[0], dir, dataset_level = dataset_level))
            ...
            for p, d, f in os.walk(session_pcap_path[key]):
                for file in f:
                    ...
                    if not os.path.getsize(current_file):
                        print("current pcap %s is 0KB and skip"%current_file)
                    else:
                        valid_files.append(current_file)

        target_all_files = valid_files
        sample_num = min(samples[label_count], len(target_all_files))
        r_files = random.sample(target_all_files, sample_num)
        label_count += 1
        for r_f in tqdm.tqdm(r_files, desc=f"Extracting features for class {key}", leave=False):
            try:
                if dataset_level == "flow":
                ...
                # Skip silently if scapy cannot read a corrupted packet
                continue
    ...
    if not os.path.exists(dataset_save_path):
        print(f"Creating directory: {dataset_save_path}")
        # exist_ok=True prevents errors if the dir was created by another process
            ...
            if X == []:
                if not ablation_flag:
                    ...
                    if sample_num > 1500:
                        ...
                        y = [label] * sample_num
                    Y.append(y)
            sample_limit = samples[label_count] if label_count < len(samples) else samples[-1]
            if sample_limit < sample_num:
                x_label = []
                for sample_index in random.sample(list(json_data[label][features[feature_index]].keys()), sample_limit):
...
def combine_dataset_json():
    dataset_name = "./traffic_pcap/splitcap/dataset-"
    # dataset vocab
    dataset = {}
    # progress
    progress_num = 8
    ...
    with open("./traffic_pcap/splitcap/dataset.json","w") as f:
        json.dump(dataset, fp=f, ensure_ascii=False, indent=4)
    return 0
...
def _pretrain_worker(file_path):
    return get_burst_feature(file_path, payload_len=64, write_to_file=False)

if __name__ == '__main__':
    import argparse
    import multiprocessing as mp
    parser = argparse.ArgumentParser()
    parser.add_argument("--pcap_dir", type=str, required=True, help="Directory containing split PCAP files")
    parser.add_argument("--pretrain", action="store_true", help="Extract burst features for pre-training")
    ...
    if args.pretrain:
        ...
        pcap_files = []
        for p, d, files in os.walk(pcap_dir):
            for file in files:
