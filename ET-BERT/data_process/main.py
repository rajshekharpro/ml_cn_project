...
# Parse arguments to allow the user's command line to work
parser = argparse.ArgumentParser()
parser.add_argument("--pcap_dir", type=str, default="../datasets/campus_source/")
parser.add_argument("--dataset_save_path", type=str, default=None, help="Optional. Defaults to pcap_dir or its parent if running on splitcap.")
parser.add_argument("--splitcap", type=bool, default=False)
args = parser.parse_args()

# Auto-resolve dataset_save_path if not provided
if args.dataset_save_path is None:
    if args.pcap_dir.rstrip('/').endswith('splitcap'):
        args.dataset_save_path = os.path.dirname(args.pcap_dir.rstrip('/')) + '/'
    else:
        args.dataset_save_path = args.pcap_dir

# Define your paths
dataset_dir = os.path.join(BASE_DIR, args.dataset_save_path)
pcap_path = os.path.join(BASE_DIR, args.pcap_dir)
dataset_save_path = os.path.join(BASE_DIR, args.dataset_save_path)

# List of all directories that MUST exist for the script to run
required_dirs = [
    dataset_dir,
    dataset_save_path,
...
# Ensure dataset_save_path ends with a slash for the script's internal concatenation
if not dataset_save_path.endswith('/'):
    dataset_save_path += '/'
...
# Ensure paths end with a slash for the script's concatenation logic
if not dataset_save_path.endswith('/'):
    dataset_save_path += '/'

# Auto-organize loose PCAP files into a '0' class folder if no class folders exist
pcap_files = glob.glob(os.path.join(pcap_path, '*.pcap')) + glob.glob(os.path.join(pcap_path, '*.pcapng'))
if pcap_files:
    class_0_dir = os.path.join(pcap_path, '0')
    os.makedirs(class_0_dir, exist_ok=True)
    for f in pcap_files:
        shutil.move(f, os.path.join(class_0_dir, os.path.basename(f)))
    print(f"Auto-organized {len(pcap_files)} loose capture files into class directory '{class_0_dir}'.")

# Auto-detect categories based on subdirectories (ignoring script output folders)
class_dirs = [d for d in os.listdir(pcap_path) if os.path.isdir(os.path.join(pcap_path, d)) and d not in ['splitcap', 'dataset', 'packet', 'result']]
if not class_dirs:
    print(f"Error: No PCAP files or class subdirectories found in {pcap_path}! Please add your .pcap files.")
    sys.exit(1)

_category = max(1, len(class_dirs))
samples = [1] * _category
...
def dataset_extract(model):
    ...
    X,Y = dataset_generation.generation(pcap_path, samples, features, splitcap=args.splitcap, dataset_save_path=dataset_save_path,dataset_level=dataset_level)

    if args.splitcap:
        print("\n--- Phase 1 (Splitting) Complete ---")
        print("To generate features, run the script again pointing to the splitcap directory without --splitcap True.")
        sys.exit(0)
    ...
    # Fix: Prevent ValueError when sample size is too small to split
    if len(x_payload) < 3:
        print("Warning: Not enough samples to split into train/test/valid. Duplicating samples for testing purposes.")
        x_payload_train, y_train = x_payload, dataset_label
        x_payload_test, y_test = x_payload, dataset_label
        x_payload_valid, y_valid = x_payload, dataset_label
    else:
